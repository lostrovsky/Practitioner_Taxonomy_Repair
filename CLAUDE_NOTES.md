...
```bash
java -jar practitioner-taxonomy-repair-1.6.6-jar-with-dependencies.jar \
...
| `v1.6.5` | 2026-07-29 | `3ab830c` | `practitioner_taxonomy_repair_v1.6.5.zip` (jar `1.6.5`) | **Removes the `-Execute` and `-LogOnlyOverride` CLI aliases.** No Java behavior change. Both were leftovers from the pre-v1.6.3 command-line workflow and had become redundant once the tool moved to edit-the-defaults: `-LogOnlyOverride` (v1.4.0) forced log-only for one run, which is exactly what `$HrpCallsLogMode = $true` already does by default, and `-Execute` (v1.6.2) existed only because `-DryRun:$false` is awkward to type. Keeping them meant two ways to express each axis -- the same inconsistency the operator flagged before v1.6.3. Removal deletes the `-Execute`/`-DryRun` contradiction check, one term from the `$LOG_ONLY` computation, and a `$LOG_ONLY_SOURCE` branch that could never fire. `run_repair.ps1` now exposes exactly one control per axis (`$DryRun`, `$HrpCallsLogMode`), and its own hint messages point at the knobs instead of the removed flags. Docs and REGRESSION_TEST.md updated (four runbook invocations lost their `-Execute`; the runbook now says to set `$DryRun = $false` in the param block). Also removed a dead `db.xref.schema` key that lingered in the local dev properties file -- the jar has not read it since v1.5.0 replaced the fabricated `cpe_xref.taxonomy` lookup. Pom bumped to 1.6.5. |
| `v1.6.6` (Latest) | 2026-07-29 | `PENDING` | `practitioner_taxonomy_repair_v1.6.6.zip` (jar `1.6.6`) | **Batches every `IN (...)` lookup.** SQL Server rejects any request carrying more than 2100 parameters, and all three lookups bound one parameter per input value, so the jar failed outright once the NPI list crossed that line: `The incoming request has too many parameters`. Hit on the first full-scale UAT dry run (17,841 NPIs). Present since v1.0.0 -- `loadHccIdsByNpi` was byte-identical to the original -- and invisible until now only because earlier runs were small (the 2026-05-22 UAT run had 167 NPIs). New `inBatches()` helper slices any list into chunks of 1000 and is applied to `loadHccIdsByNpi`, `loadMasterSnapshotsByNpi`, and `lookupTaxonomyNames`; it logs the batch count when it engages. Verified with a 2500-NPI dry run (3 batches per query, exit 0, same decisions as the unbatched 5-NPI run). The INSERT path was audited and needed no change: `addBatch`/`executeBatch` sends each row's parameter set as its own execution, and no `useBulkCopyForBatchInsert` is set, so the 2100 cap applies per row (4-6 params) rather than in aggregate. Note what is NOT fixed by this: NPPES lookups remain sequential at ~500ms each, so a full 17,841-NPI run is ~2.5 hours, and `persistRun` still wraps all inserts in one transaction. Scope filtering matters more than the chunking. Pom bumped to 1.6.6. |
...
Release `v1.6.6` shipped 2026-07-29 (marked Latest). Fixes a scale bug present since v1.0.0: every `IN (...)` lookup bound one parameter per input value, so the jar died on SQL Server's 2100-parameter ceiling once the NPI list grew. The first full-scale UAT dry run (17,841 NPIs) failed with `The incoming request has too many parameters` inside `loadHccIdsByNpi`. Earlier runs never reached it -- the 2026-05-22 UAT run had 167 NPIs, and `cpe_master` has grown roughly 100x since. Nothing in v1.6.x caused it; `loadHccIdsByNpi` was byte-identical to v1.0.0.

The dry-run guard behaved correctly throughout the failure: no writes, jar exited 1, orchestrator caught it, released the lock, and reported `Status: FAILED` with the cause.

**Bug window, now pinned.** The pre-rank reset was introduced in extractor commit `c230d6f` (2026-03-20) and removed in `ab450a9` (2026-04-25 08:42 EDT = 12:42 UTC). Pipeline v1.4.1 was published 2026-04-25 12:45 UTC -- three minutes later -- and the operator confirmed it was deployed to UAT the same day. Since the reset predates the pipeline's first release (v1.0.0, 2026-04-10), **every pipeline release from v1.0.0 through v1.4.0 carries the bug; v1.4.1 onward is clean.** Note that pipeline tags v1.4.0/v1.4.1/v1.4.2 all point at the same commit (`b2ac38b`, 04-23) -- they differed only in bundled jars, so tag commit dates are misleading here and the GitHub release publish dates are what matter. `run_pipeline.ps1` prints `Pipeline version:` at startup and in its summary, so affected runs are identifiable from the logs.

**Scoping the repair.** `cpe_master.practitioner_taxonomy` has `created_time` (fully populated, no NULLs) and no `run_id`, so no join to `cpe_load.load_run` is needed:

```sql
SELECT DISTINCT npi FROM cpe_master.practitioner_taxonomy
WHERE taxonomy_source = 'NPPES' AND created_time < '2026-04-26'
```

Cut at 04-26 rather than 04-25: the fixed build went out mid-day on the 25th, so rows written that morning are still suspect. Over-selecting is free because matching NPIs are recorded as `skipped`; under-selecting leaves wrong primaries in HRP. Whether the daily pipeline refreshes `created_time` on re-load is unknown, but both readings give the correct answer -- if it refreshes, re-loaded practitioners already got a corrected primary and rightly drop out.
