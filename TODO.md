...
- [ ] **Apply DDL to production DB:** `sqlcmd -S <server> -d <db> -U <user> -P <pwd> -i sql/create_cpe_repair_objects.sql`. Idempotent; safe to re-run (fresh installs only as of v1.6.0 — the embedded v1.x → v1.5 migration block was removed). To rebuild an old-schema install, run `sql/drop_cpe_repair_objects.sql` first.
...
- [x] **v1.6.2 GitHub Release** (2026-07-27, commit `PENDING`, marked Latest) -- safety + auditability release for `run_repair.ps1`; no Java behavior change. **Dry-run is now the default** (`[switch]$DryRun = $true`); real runs require the new `-Execute` switch, and `-Execute` + `-DryRun` together is a hard error. Resume mode honors the guard -- `-RunId <n>` without `-Execute` verifies the run then stops before the loader (without this the new default would have been dangerous, since the dry-run exit sits inside the `-not $RESUME_MODE` block and resume would have fallen through to a live loader call). Transcript now records the reconstructed invocation at STEP 1 and in the RUN SUMMARY, plus explicit `DB writes:` / `HRP calls:` lines -- rebuilt from `$PSBoundParameters`, not `$MyInvocation.Line` (which captures the caller's whole command line and would leak `$env:SQLCMDPASSWORD='...'` into the log). Mode banners for dry-run / log-only / live. `build_package.ps1` stages `sql/rebuild_cpe_repair_objects.sql` into the zip. Docs updated across README / INSTALL.txt / install.ps1 / REGRESSION_TEST.md. Pom bumped to 1.6.2.
