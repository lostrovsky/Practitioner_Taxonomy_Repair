...
-- repairs. Does NOT touch cpe / cpe_load / cpe_master or any
-- existing cpe_xref objects.
-- ============================================================
...
-- 2. Batch table -- one row per repair invocation
IF OBJECT_ID('cpe_repair.batch', 'U') IS NULL
BEGIN
    CREATE TABLE cpe_repair.batch (
        ...
        CONSTRAINT pk_repair_batch PRIMARY KEY (batch_id)
    );
END
GO

-- 3. Per-practitioner-per-batch row -- the "entity" the loader marks loaded.
--    entity_id is the post-call SQL target. UNIQUE on (batch_id, npi) prevents
--    accidental duplicate staging within one batch.
...
BEGIN
    CREATE TABLE cpe_repair.practitioner_repair (
        ...
        CONSTRAINT uq_repair_practitioner_batch_npi UNIQUE (batch_id, npi)
    );
    CREATE INDEX ix_repair_practitioner_batch ON cpe_repair.practitioner_repair(batch_id);

    ALTER TABLE cpe_repair.practitioner_repair WITH NOCHECK
        ADD CONSTRAINT fk_repair_practitioner_batch
            FOREIGN KEY (batch_id) REFERENCES cpe_repair.batch(batch_id);
END
GO
...
--    primary/secondary slots -- same row shape as cpe_load's amend TVF.
--    The loader's existing template can render it without changes.
CREATE OR ALTER FUNCTION cpe_repair.fn_get_practitioner_taxonomy_repair_for_batch_id(@batch_id BIGINT)
RETURNS TABLE
AS
RETURN
(
    WITH PTXN (entity_id, batch_id, practitioner_hcc_id, seq_num, taxonomy_code, taxonomy_name, is_primary, is_secondary) AS
    (
        SELECT pr.entity_id,
               pr.batch_id,
    ...
    SELECT
        pr.entity_id,
        pr.batch_id AS run_id,                 -- mapped to ${RUN_ID} placeholder for the loader; semantically a batch_id
        pr.practitioner_hcc_id,
